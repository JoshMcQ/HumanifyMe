...
2. Confirm the version is synchronized across `package.json`, plugin manifests,
   ...
   ```bash
   npm run check:release-version -- --tag v0.2.2 --ensure-unpublished
   ```
...
6. Verify the public artifact rather than relying on the workflow result alone:
   ...
   ```bash
   npm view humanifyme@0.2.2 version dist.integrity dist.tarball
   npx -y humanifyme@0.2.2 --version
   echo "At its core, this robust approach paves the way." | npx -y humanifyme@0.2.2 analyze
   ```
