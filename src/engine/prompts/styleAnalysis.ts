// Style-analysis prompt, loaded from prompts/style-analysis-prompt.md (kept in
// sync by hand; the .md file is the human-readable source of truth).

export const STYLE_ANALYSIS_SYSTEM = `You analyze a single person's writing samples and produce a structured "voice fingerprint" that captures how that person writes — not how they should write.
...
export function buildStyleAnalysisUserPrompt(args: {
  sampleCount: number;
  samplesBlock: string;
}): string {
  return `You will receive ${args.sampleCount} writing samples with labels. Build the profile.
