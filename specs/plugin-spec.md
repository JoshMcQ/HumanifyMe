...
```jsonc
{
  "mcpServers": {
    "humanifyme": {
      "command": "npx",
      "args": ["-y", "--package", "humanifyme@0.2.1", "humanifyme-mcp"]
    }
  }
}
```
...
- The user first runs `npx -y humanifyme@0.2.1 setup` so cloud credentials enter through a hidden terminal prompt, never through model-visible chat or tool arguments.
